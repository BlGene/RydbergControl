...
def single_test(algo,toal_evals=1200, pop_size = 20):
    ...
    while isl.problem.get_evals() < total_evals:
        isl.evolve(1)
        isl.join()
        best.append(isl.population.champion.f[0])
        best_x.append(isl.population.champion.x)
    else:
...
def run(n_tri,n_gen,pop_size):
    ...
    for algo in algo_list:
            ...
            for i in range(number_of_trials):
                print ".",
                trials_best_list.append(single_test(algo,total_evals,pop_size))
            
            import pdb
            pdb.set_trace()
            trials_means = numpy.mean(numpy.array(trials_best_list),axis=0)
            algo_means_list.append(trials_means)
